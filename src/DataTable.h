...
class DataTable
{
    ...
    public:
        ...
        void GetDataWhere(const std::string &clause, std::vector<Data<Value>*> &data_list);
        ...
        Value* GetPrimaryKey(const Data<Value>* data);
        ...
        bool CheckPrimaryKey(const std::vector< Attribute<Value> >& attributes);

        bool CheckPrimaryKey(const Attribute<Value> & attribute);

        //check whether there are uninitialized attribute which should be not null.
        bool CheckNotNullKey(const std::vector< Attribute<Value> >& attributes);

        //check whether there are unexist attribute name in the input.
        bool CheckAttributeName(const std::vector< Attribute<Value> >& attributes);

        bool CheckAttributeName(const Attribute<Value>& attribute);
